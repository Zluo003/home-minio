...
The legacy manifest pull and the lifecycle worker share the same signed path-style HTTP client for MinIO. Lifecycle scheduling reloads the values saved by the web console before starting new work. If logs report `MinIO ... HTTP 403`, synchronize the currently saved service account and policy into MinIO, then recreate the Web API container:
