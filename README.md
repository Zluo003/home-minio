...
If old database rows still point to OSS objects that were already deleted, those rows will return `404`. The pull script records them in `MEDIA_PULL_WORK_DIR/failed-*.jsonl` and still mirrors every successfully downloaded file into MinIO, so stale rows do not block valid media from being archived. After pull finishes, update NewWaule database without downloading files through the Hong Kong server:
